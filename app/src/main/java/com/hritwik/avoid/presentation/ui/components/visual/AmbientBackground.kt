...
fun AmbientBackground(
    ...
    BoxWithConstraints(
        ...
        val primaryX = screenWidth * 0.3f
        val primaryY = screenHeight * 0.4f
        val secondaryX = screenWidth * 0.7f
        val secondaryY = screenHeight * 0.6f

        val primaryBrush = remember(extractedColors, intensity) {
            if (extractedColors.size >= 2) {
                ...
                Brush.radialGradient(
                    colors = listOf(
                        extractedColors.getOrElse(1) { Color(0xFF6366F1) }.copy(alpha = intensity * 0.8f),
                        Color.Transparent
                    ),
                    center = Offset(primaryX, primaryY),
                    radius = (screenWidth.coerceAtLeast(screenHeight) * 0.8f)
        ...
        val secondaryBrush = remember(extractedColors, intensity) {
